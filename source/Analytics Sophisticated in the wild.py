...
parser.add_argument('--labels', type=str, default='dev', help='Labels (dev)')
...
parser.add_argument('--dataset', type=str, default='inthewild', help='Dataset name')
...
def create_bar_figure(check, data_list):
    method_names = []
    correct = []
    wrong = []
...
trial = pd.read_csv("data/in_the_wild/meta.csv", sep=',')
trial.columns = ["filename", "speaker", "method_type"]
trial["method_type"] = trial["method_type"].apply(lambda x: 'bonafide' if x == 'bona-fide' else x)
trial["is_genuine"] = trial["method_type"].apply(lambda x : 1 if x == "bonafide" else 0)
trial["utterance"] = trial["filename"].apply(lambda x : x.split(".")[0])
def get_filename(file):
    file = f"data/in_the_wild/{file}"
    file = os.path.abspath(file)
    return file
trial["filename"] = trial["filename"].apply(get_filename)
data_list = trial
